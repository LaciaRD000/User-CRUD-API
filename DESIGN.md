...
- Rust + Axum で REST API の設計・実装パターンを学ぶ（学習プロジェクト）
- ユーザーの CRUD 操作を提供する
- JWT (HS256) によるステートレス認証と、リフレッシュトークンによるセッション管理を実装する
- パスワードの安全な保存（Argon2id）、レートリミット、CORS 等のセキュリティ基盤を構築する
- RGBC サイクル（Red → Green → Blue → Commit）による TDD を実践する

## Non-Goals（今回はやらないこと）

- **メール送信**: パスワードリセット、メールアドレス確認等のメールフローは実装しない
- **フロントエンド**: API のみ提供し、UI は作らない
- **水平スケーリング**: Snowflake ID の `machine_id` は手動設定。複数インスタンスの自動調整は行わない
- **外部 IdP 連携**: OAuth2 / OpenID Connect（Google, GitHub 等）によるソーシャルログインは対象外
- **監査ログ・イベント発行**: ユーザー操作の永続的な監査証跡やメッセージキューへのイベント発行は行わない
...
- `CorsLayer` — 全オリジン許可、GET/POST/PUT/DELETE メソッド許可、`Authorization` / `Content-Type` ヘッダー許可、`x-request-id` を `expose_headers` で公開（フロントエンドでリクエスト ID を参照可能にする）
- `CompressionLayer` — レスポンスの gzip 圧縮
- `DefaultBodyLimit` — リクエストボディのサイズ上限 (5MB)。`axum::extract::DefaultBodyLimit` が提供（tower-http ではなく Axum 本体）
...
**ミドルウェア実行順序**（リクエスト処理の流れ）:

`ServiceBuilder` では **最後に追加したレイヤーが最も外側**（リクエストを最初に処理）になる。

```
リクエスト
  │
  ▼
NormalizePathLayer ─── 末尾スラッシュ除去（Router の外側でラップ）
  │
  ▼
SetRequestIdLayer ─── x-request-id 付与（ServiceBuilder 最後 = 最も外側）
  │
  ▼
HelmetLayer ────────── セキュリティヘッダー
  │
  ▼
TimeoutLayer ───────── タイムアウト監視開始
  │
  ▼
DefaultBodyLimit ───── ボディサイズチェック
  │
  ▼
CompressionLayer ───── レスポンス圧縮の準備
  │
  ▼
CorsLayer ──────────── CORS ヘッダー
  │
  ▼
TraceLayer ─────────── ログスパン作成（x-request-id は付与済み ✓）
  │                    （ServiceBuilder 最初 = 最も内側）
  ▼
GovernorLayer ──────── レートリミット（ルートごとに適用）
  │
  ▼
ハンドラー
```
...
|--------------|-------------|-------------|------|
| 認証ルート (`/auth/*`) | 1 | 5 | Argon2 が重い + ブルートフォース防止のため厳しく制限 |
...
- `password_hash` — Argon2id でハッシュ化したパスワードを PHC 文字列形式で格納。`DEFAULT ''` は `ALTER TABLE ADD COLUMN` で既存行にカラムを追加した際の互換性のために設定。新規テーブル作成時は `DEFAULT` を省略してもよい
...
```sql
-- 既存の email UNIQUE 制約を削除し、case-insensitive な関数インデックスに統一する
-- ※ 両方が共存すると map_unique_violation_to_conflict で2つの制約名を
--   チェックする必要があり、保守が複雑になるため、統一を推奨
...
> **注意**: `users_email_key`（元の `UNIQUE` 制約）を削除せず残す場合、`map_unique_violation_to_conflict` で両方の制約名をチェックする必要がある（現在の実装はこのケースにも対応済み）。新規環境では `users_email_lower_key` のみ作成することを推奨する。
...
└── dummy_password_hash         : String                          — login のタイミング差対策用ダミー Argon2 ハッシュ
```
...
| 実装 | 説明 |
|------|------|
| `impl IntoResponse for ApiError` | ApiError を Axum の HTTP レスポンスに変換する。ステータスコードと `{"error": "メッセージ"}` の JSON ボディを返す。`Forbidden` は `{"error": "Forbidden"}` を返す。**`Internal` はクライアントに詳細を返さず `{"error": "An internal error occurred"}` 固定**（DB エラー等の内部情報を漏洩しない）。詳細は `tracing::error!` でサーバーログにのみ記録する |

**ヘルパー関数**:

| 関数 | シグネチャ | 説明 |
|------|-----------|------|
| `map_unique_violation_to_conflict` | `pub fn map_unique_violation_to_conflict(code: Option<&str>, constraint: Option<&str>) -> Option<ApiError>` | PostgreSQL の UNIQUE 違反（SQLSTATE 23505）を検出し、email 制約（`users_email_key` / `users_email_lower_key`）の場合のみ `ApiError::Conflict("email already exists")` を返す。それ以外の制約では `None`（呼び出し元で `Internal` にフォールバック）。`routes/auth.rs` と `routes/users.rs` の両方から使用する共通ヘルパー |
...
| `validate_password` | `pub fn validate_password(password: &str) -> Result<(), String>` | 空チェック、8文字以上、1024バイト以下。失敗時はエラーメッセージを返す |
...
2. `tokio::task::spawn_blocking` で Argon2id によるパスワードハッシュ化を実行（CPU バウンド処理を Tokio のワーカースレッドからブロッキング専用スレッドプールに逃がす）
3. `state.snowflake.lock().unwrap().generate()` で Snowflake ID 生成
...
3. **タイミング差対策**: ユーザーが見つからない場合でも `state.dummy_password_hash`（起動時に生成したダミー Argon2 ハッシュ）に対して照合を実行する。これにより email の存在有無でレスポンス時間に差が出ないようにする
4. `tokio::task::spawn_blocking` で Argon2 によるパスワード照合を実行。不一致またはユーザー不在なら `ApiError::Unauthorized`
...
### `rate_limit.rs` — レート制限のキー抽出

**役割**: `GovernorLayer` が使う IP アドレスの抽出方法を、環境変数 `RATE_LIMIT_IP_MODE` に応じて切り替える

**列挙型**:

```
RateLimitIpMode (Debug, Clone, Copy, PartialEq, Eq)
├── Peer  — peer IP のみ使用。ヘッダーを一切信頼しない（直アクセス環境向け、既定値）
└── Smart — Forwarded / X-Forwarded-For / X-Real-Ip から取得し、無ければ peer IP にフォールバック
            （信頼できるリバースプロキシ配下でのみ使用）
```

**構造体**:

```
RateLimitIpKeyExtractor (Debug, Clone, Copy)
└── mode : RateLimitIpMode — 抽出モード
```
...
| 関数 | シグネチャ | 説明 |
|------|-----------|------|
| `RateLimitIpMode::parse` | `pub fn parse(s: &str) -> Option<Self>` | 文字列 (`"peer"` / `"smart"`) を `RateLimitIpMode` に変換。大文字小文字を無視し、不正値は `None` |
| `RateLimitIpKeyExtractor::new` | `pub fn new(mode: RateLimitIpMode) -> Self` | 指定モードで初期化 |

**トレイト実装**:

| 実装 | 説明 |
|------|------|
| `impl KeyExtractor for RateLimitIpKeyExtractor` | `mode` に応じて `Peer` なら `PeerIpKeyExtractor`（+ `SocketAddr` フォールバック）、`Smart` なら `SmartIpKeyExtractor` に委譲。抽出失敗時は `GovernorError::UnableToExtractKey` を返す |

> **運用上の注意**: 直アクセス環境で `smart` モードを使うと、クライアントが `X-Forwarded-For` ヘッダーを偽装してレートリミットを回避できる。信頼できるプロキシ配下でのみ `smart` を使用すること。
...
| `sqlx` | 0.8 (features: runtime-tokio, postgres, chrono) | PostgreSQL 非同期クライアント。`chrono` feature で `DateTime<Utc>` ↔ `TIMESTAMPTZ` の変換に対応。本プロジェクトでは文字列クエリ (`query()` / `query_as()`) を使用しており、コンパイル時クエリチェックマクロ (`query!()`) は使用していない |
...
| `argon2` | 0.5 | パスワードのハッシュ化 (`PasswordHasher`) と照合 (`PasswordVerifier`)。Argon2id アルゴリズムを使用（OWASP 推奨）。bcrypt の 72 バイト入力長制限がなく、GPU 耐性も高い |
...
**`map_unique_violation_to_conflict`** ヘルパー（`errors.rs` に定義、`routes/auth.rs` と `routes/users.rs` から共用）:
...
15. **`Cargo.toml`** — `jsonwebtoken`, `argon2`, `chrono`, `uuid`, `sha2`, `hmac`, `hex` を追加。`sqlx` の features に `chrono` を追加
...
32. **`main.rs`** — ミドルウェアを `ServiceBuilder` に積む（**最後に追加したレイヤーが最も外側＝リクエストを最初に処理**）:
    1. `TraceLayer` — `make_span_with` で `x-request-id` をスパンに記録（最も内側）
    2. `CorsLayer` — CORS ヘッダー（`expose_headers` で `x-request-id` を公開）
    3. `CompressionLayer` — gzip 圧縮
    4. `DefaultBodyLimit` — ボディサイズ上限 (5MB)
    5. `TimeoutLayer` — タイムアウト (10秒)
    6. `HelmetLayer` — セキュリティヘッダー
    7. `SetRequestIdLayer` — リクエスト ID 付与（最も外側＝最初に実行。これにより後続の TraceLayer でIDを参照可能）
